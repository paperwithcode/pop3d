...
import os
def train(env_id, num_timesteps, seed, args=None):
    ...
    pposgd_simple.learn(env, policy_fn,
            ...
            gamma=0.99, lam=0.95, schedule='linear', args=args
        )
    env.close()
...
def main():
    ...
    logger.configure(args.env + '_seed_' + str(args.seed) + '_nopen' + '_pg' + str(args.pg_rate) if not args.use_penal
                     else args.env + '_seed_' + str(args.seed) + '_pen' + '_pg_base' + str(args.pg_rate), ['log', 'tensorboard'])
